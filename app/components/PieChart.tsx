...
export default function PieChart({
  ...
  const legendItemGap =
    dataLength <= 4 ? 60 : dataLength <= 6 ? 40 : 25;

  const containerMinHeight =
    dataLength <= 4
      ? 380
      : 380 + Math.min((dataLength - 4) * 20, 80);
  ...
  const dataWithColors = normalizedData.map(
    (item, index) => ({
      ...item,
