...
const getFontSizeFromElement = (
  ...
  try {
    ...
    if (!Number.isNaN(computedSize) && computedSize > 0) return computedSize;
  } catch {
    // fallback
  }
...
const getDominantFontSize = (
  ...
  if (textElements.length === 0) return fallback;

  const sizes: number[] = [];
  for (const textEl of Array.from(textElements)) {
    const size = getFontSizeFromElement(textEl as SVGTextElement, fallback);
...
const preserveTextGeometry = (
  ...
  const sourceTextNodes = Array.from(
    sourceSvg.querySelectorAll("text"),
  ) as SVGTextElement[];

  const clonedTextNodes = Array.from(
    cloneSvg.querySelectorAll("text"),
  ) as SVGTextElement[];

  sourceTextNodes.forEach((sourceText, index) => {
    const clonedText = clonedTextNodes[index];
    ...
    try {
      ...
      clonedText.setAttribute("data-export-bbox-x", String(bbox.x));
      clonedText.setAttribute("data-export-bbox-y", String(bbox.y));
      clonedText.setAttribute("data-export-bbox-width", String(bbox.width));
      clonedText.setAttribute("data-export-bbox-height", String(bbox.height));
      ...
      const anchor =
        ...
        computed.textAnchor ||
        "start";
      clonedText.setAttribute("text-anchor", anchor);

      const baseline = sourceText.getAttribute("dominant-baseline");
      if (baseline) {
        clonedText.setAttribute("dominant-baseline", baseline);
      }
    } catch {
...
const textElementToPaths = (
  ...
  const textAnchor =
    textNode.getAttribute("text-anchor") || computed.textAnchor || "start";
  ...
  const containsRTL = tokens.some((token) => token.isRTL);
  const containsOnlyNumeric = tokens.every(
    (token) => token.isNumeric || !token.text.trim(),
  );
  const orderedTokens =
    containsRTL && !containsOnlyNumeric ? [...tokens].reverse() : tokens;
  ...
  for (const { glyphs, positions } of shapedRuns) {
    for (let i = 0; i < glyphs.length; i++) {
      ...
      if (bbox) {
        ...
        const top = yOffset - bbox.maxY * scale;
        const bottom = yOffset - bbox.minY * scale;
  ...
  for (const { glyphs, positions } of shapedRuns) {
    for (let i = 0; i < glyphs.length; i++) {
      ...
      const yOffset = (position?.yOffset || 0) * scale;
      const glyphX = cursorX + xOffset;
      const glyphY = baselineY + yOffset;
      ...
      let transform =
        `translate(${glyphX}, ${glyphY}) ` + `scale(${scale}, ${-scale})`;
...
const convertTextElementsToPaths = async (
  svgClone: SVGSVGElement,
  font?: any,
) => {
  const loadedFont = font || (await loadFont());
  ...
  for (const textNode of textNodes) {
    try {
      const paths = textElementToPaths(textNode, loadedFont);
...
const buildExportSVG = async (
  ...
  preserveTextGeometry(svgElement, svgClone);
  ...
  const vbWidth =
    Number.isFinite(values[2]) && values[2] > 0 ? values[2] : originalWidth;
  const vbHeight =
    Number.isFinite(values[3]) && values[3] > 0 ? values[3] : originalHeight;

  const legendItems = extractLegendData(chartElement);
  const hasCustomLegend = legendItems.length > 0;
  ...
  const totalViewBoxWidth = vbWidth + legendViewBoxWidth;
  ...
  if (hasCustomLegend) {
    ...
    const legendFontSize = Math.min(chartFontSize, 40);

    const markerWidth = legendFontSize * 0.58;
    const markerHeight = legendFontSize * 0.34;
    const markerTextGap = legendFontSize * 0.28;
    const rowHeight = legendFontSize * 1.18;
    const rowGap = legendFontSize * 0.18;

    const legendLeft = vbX + vbWidth + legendFontSize * 0.6;
    const legendRight = vbX + totalViewBoxWidth - legendFontSize * 0.5;
    const availableLegendWidth = Math.max(1, legendRight - legendLeft);

    const totalLegendHeight =
      legendItems.length * rowHeight +
      Math.max(0, legendItems.length - 1) * rowGap;

    const legendBottomPadding = legendFontSize * 0.5;
    let currentY =
      vbY + vbHeight - totalLegendHeight - legendBottomPadding;
    ...
    for (const item of legendItems) {
      ...
      rect.setAttribute("y", String(centerY - markerHeight / 2));
      rect.setAttribute("width", String(markerWidth));
      rect.setAttribute("height", String(markerHeight));

      const radius = Math.min(4, markerHeight / 3);
      ...
      text.setAttribute(
        "data-max-width",
        String(availableLegendWidth - markerWidth - markerTextGap),
      );
  ...
  await convertTextElementsToPaths(svgClone, font);
...
const renderSVGToCanvas = (
  ...
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => {
      URL.revokeObjectURL(url);
      reject(new Error("Could not render SVG for raster export"));
    };
...
export const downloadChart = async (
  ...
  try {
    ...
    // Detect if the target chart is a Sankey chart
    const isSankey = Boolean(
      chartElement.getAttribute("data-chart-type") === "sankey" ||
      chartElement.querySelector('[data-chart-type="sankey"]') ||
      chartElement.querySelector('[data-tight-svg-export="true"]') ||
    ...
    const exportSVG = await buildExportSVG(
      chartElement,
      targetWidth,
      targetHeight,
      isSankey,
    ...
    switch (format) {
      case "png":
        await downloadAsPNG(exportSVG, fileName);
        break;
      case "jpg":
        await downloadAsJPG(exportSVG, fileName);
        break;
      case "svg":
        downloadAsSVG(exportSVG, fileName);
        break;
      case "pdf":
        await downloadAsPDF(exportSVG, fileName);
        break;
    }
  } catch (error) {
...
const downloadAsSVG = (exportSVG: BuiltExportSVG, title: string) => {
  const blob = new Blob([exportSVG.svgString], {
    type: "image/svg+xml;charset=utf-8",
  });
  downloadBlob(blob, `${title}.svg`);
};

const downloadAsPNG = async (exportSVG: BuiltExportSVG, title: string) => {
...
const downloadAsJPG = async (exportSVG: BuiltExportSVG, title: string) => {
  const canvas = await renderSVGToCanvas(
    exportSVG.svgString,
    exportSVG.width,
    exportSVG.height,
  ...
  await new Promise<void>((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (!blob) {
          reject(new Error("Could not create JPG"));
          return;
        }
...
const downloadAsPDF = async (exportSVG: BuiltExportSVG, title: string) => {
  const canvas = await renderSVGToCanvas(
    exportSVG.svgString,
    exportSVG.width,
    exportSVG.height,
  ...
  const pdfWidth = exportSVG.width;
  const pdfHeight = exportSVG.height;
